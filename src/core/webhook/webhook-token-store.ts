...
export class WebhookTokenStore {
  ...
  save(token: Omit<WebhookToken, "last_used">): void {
    this.db.prepare<[string, string, string, string, string, number, string], void>(
      `INSERT INTO webhook_tokens (id, agent_id, source, token_hash, label, enabled, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      token.id,
      token.agent_id,
    ...
    logger.debug("webhook-token-store", "Webhook token saved", {
      metadata: { id: token.id, agent_id: token.agent_id, source: token.source },
    });
  }

  /** Return all active tokens for an agent (enabled=1). */
  findByAgent(agentId: string): WebhookToken[] {
    ...
    const rows: Row[] = this.db
      .prepare<[string], Row>(
        "SELECT * FROM webhook_tokens WHERE agent_id = ? AND enabled = 1 ORDER BY created_at ASC",
      )
      .all(agentId);
